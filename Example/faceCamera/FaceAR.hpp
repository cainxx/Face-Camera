...
#ifndef _TRACKER_FaceAR_h_
#define _TRACKER_FaceAR_h_
#include "IO.hpp"
  ...
  */
  ...
  public:
    ...
    Track(cv::Mat &im,          //grayscale image to track
	  FaceARParams* params=NULL){   //additinal parameters
      _timer.start_frame(); 
    ...
    NewFrame(cv::Mat &im,          //grayscale image to track
	     FaceARParams *params=NULL)=0; //additinal parameters 
    virtual void 
  ...
  //============================================================================
  /** IO functions */
  FaceAR* //contains pointer to facetracker obj on return, null on failure
  LoadFaceAR(const char* fname); //filename containing facetracker model

  // TODO The FaceAR and FaceARParams interface really needs
  // to be improved/removed. Why is there no FaceAR argument? The
  // API reads like the parameters are not associated with the type of
  // face tracker which isn't the case.

  FaceARParams *  //contains pointer to facetracker param obj on return, null on failure
  LoadFaceARParams(const char* fname); //file containing facetracker params
  //============================================================================

  std::string DefaultFaceARModelPathname();
  std::string DefaultFaceARParamsPathname();

  FaceAR * LoadFaceAR(); // load the default face tracker model.
  FaceARParams *LoadFaceARParams();   // load the default face tracker parameters.
