...
void IOBinary::ReadMat(std::ifstream &s, cv::Mat &M)
{
	...
	M = cv::Mat(r,c,t);
	s.read(const_cast<char*>(reinterpret_cast<const char*>(M.datastart)), M.total()*M.elemSize());
...
void IOBinary::WriteMat(std::ofstream &s, cv::Mat &M)
{
	...
	//	s << M.rows << " " << M.cols << " " << M.type();
	s.write(const_cast<char*>(reinterpret_cast<const char*>(M.datastart)), M.total()*M.elemSize());
