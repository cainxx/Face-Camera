                   ...
                   void* data)
{
    ...
    for(size_t witer = 0; witer < wSize.size(); witer++){
        ...
        //transform landmark candidates to image frame
        __block  std::vector<cv::Mat> xloc(n);
        __block  std::vector<cv::Mat> yloc(n);
...
#pragma omp parallel for
        ...
        //optimise
        cshape_.copyTo(bshape_);
//        this->OptimizePrior(xloc,yloc,idx,wsize,nIter,fTol,clamp,1,lambda,im,pfunc,data);
        e1 = cv::getTickCount();
        this->OptimizePrior(xloc,yloc,idx,wSize[witer],nIter,fTol,clamp,0,lambda,im,pfunc,data);
        e2 = cv::getTickCount();
//        t = (e2 - e1)/cv::getTickFrequency();
//        printf(" OptimizePrior %f \n",t );
    }return;
}
//=============================================================================
...
#pragma omp parallel for
        ...
        dispatch_apply(n, queue, ^(size_t j) {
            ...
            if(discard){
                ...
                ms_.db(i,0) = 0.0; ms_.db(i+n,0) = 0.0; return ;
            }
