...
#import "NewStickerCameraViewController.h"
...
@interface NewStickerCameraViewController ()<GPUImageVideoCameraDelegate,AVCaptureMetadataOutputObjectsDelegate>{
    UIView *faceView;
    UIView *leftEyeView;
    UIView *leftEye;
    UIView *mouth;
...
@property NSInteger frame_count;
@property BOOL faceThinking;
...
@property GPUImagePicture *widgetImg1;
@property GPUImagePicture *widgetImg2;
@property GPUImagePicture *widgetImg3;
@property GPUImagePicture *widgetImg4;
@property GPUImagePicture *widgetImg5;
@property GPUImagePicture *widgetImg6;
...
@property GPUImagePeopleFilter *peopleFilter5;
@property GPUImagePeopleFilter *peopleFilter6;
...
@property GPUImageNormalBlendFilter *blendfilter;
...
@property NSTimeInterval lastTrackerTime;
...
@implementation NewStickerCameraViewController
...
- (void)viewDidLoad {
    ...
    self.lowModel = false;
    self.time = 0;
    self.lastTrackerTime = [NSDate timeIntervalSinceReferenceDate];
    ...
    self.primaryView = [[GPUImageView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH , SCREEN_HEIGHT)];
    self.primaryView.backgroundColor = [UIColor colorWithARGBHex:0x00ffffff];
    [self.cameraView addSubview:self.primaryView];
    
    UITapGestureRecognizer *doubleTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(doubleTap:)];
    [doubleTap setNumberOfTapsRequired:2];
    [self.primaryView addGestureRecognizer:doubleTap];
    ...
    [self.primaryView addGestureRecognizer:singleTap];
    ...
    self.peopleFilter = [GPUImagePeopleFilter new];
    self.peopleFilter.imgSize = self.cameraSize;
    self.peopleFilter2 = [GPUImagePeopleFilter new];
    self.peopleFilter2.imgSize = self.cameraSize;
    self.peopleFilter3 = [GPUImagePeopleFilter new];
    self.peopleFilter3.imgSize = self.cameraSize;
    self.peopleFilter4 = [GPUImagePeopleFilter new];
    self.peopleFilter4.imgSize = self.cameraSize;
    self.peopleFilter5 = [GPUImagePeopleFilter new];
    self.peopleFilter5.imgSize = self.cameraSize;
    self.peopleFilter6 = [GPUImagePeopleFilter new];
    self.peopleFilter6.imgSize = self.cameraSize;
    self.backgroundFilter = [GPUImageStickerFilter new];
    
    self.facear =[[FaceARDetectIOS alloc] init :self.lowModel];
    ...
    self.stickersData = @[
        @{
            ...
            @"stickers":@[
                ...
                @{@"icon":@"skin_leopard.png" ,@"path":@"stickers/skin_leopard"},
        ...
        @{
            ...
            @"stickers":@[
                ...
                @{@"icon":@"100102.png" ,@"path":@"stickers/100102"},
                @{@"icon":@"100103.png" ,@"path":@"stickers/100103"},
                @{@"icon":@"100104.png" ,@"path":@"stickers/100104"},
                @{@"icon":@"100105.png" ,@"path":@"stickers/100105"},
...
-(void)stickerTap:(UIView *)tapView :(NSDictionary *)sticker{
    ...
    self.widgetImg1 = self.placeholderImg;
    self.widgetImg2 = self.placeholderImg;
    self.widgetImg3 = self.placeholderImg;
    self.widgetImg4 = self.placeholderImg;
    self.widgetImg5 = self.placeholderImg;
    self.widgetImg6 = self.placeholderImg;
    ...
    for (NSDictionary *item in self.stickerConfig[@"items"]) {
        ...
        if(i == 0){
            ...
            [self.laseFilter addTarget:self.peopleFilter];
            self.laseFilter = self.peopleFilter;
            [self.widgetImg1 addTarget:self.laseFilter atTextureLocation:1];
        }
        else if(i == 1){
            [self.laseFilter addTarget:self.peopleFilter2];
            [self.peopleFilter2 setStickerParams:parames];
            self.laseFilter = self.peopleFilter2;
            [self.widgetImg2 addTarget:self.peopleFilter2 atTextureLocation:1];
        }
        else if(i == 2){
            [self.laseFilter addTarget:self.peopleFilter3];
            [self.peopleFilter3 setStickerParams:parames];
            self.laseFilter = self.peopleFilter3;
            [self.widgetImg3 addTarget:self.peopleFilter3 atTextureLocation:1];
        }
        else if(i == 3){
            [self.laseFilter addTarget:self.peopleFilter4];
            [self.peopleFilter4 setStickerParams:parames];
            self.laseFilter = self.peopleFilter4;
            [self.widgetImg4 addTarget:self.peopleFilter4 atTextureLocation:1];
        }
        else if(i == 4){
            [self.laseFilter addTarget:self.peopleFilter5];
            [self.peopleFilter5 setStickerParams:parames];
            self.laseFilter = self.peopleFilter5;
            [self.widgetImg5 addTarget:self.peopleFilter5 atTextureLocation:1];
        }else if(i == 5){
            [self.laseFilter addTarget:self.peopleFilter6];
            [self.peopleFilter6 setStickerParams:parames];
            self.laseFilter = self.peopleFilter6;
            [self.widgetImg6 addTarget:self.peopleFilter6 atTextureLocation:1];
        }
        i++;
    }
    ...
    [self.videoCamera addTarget:self.firstFilter];
    [self.laseFilter addTarget:self.primaryView];
}

-(void)resetFilters{
    [self clearFilters];
    [self.firstFilter addTarget:self.primaryView];
    [self.videoCamera addTarget:self.firstFilter];
}
...
-(void)clearFilters{
    ...
    [self.peopleFilter5 removeAllTargets];
    [self.peopleFilter6 removeAllTargets];
    [self.backgroundFilter removeAllTargets];
    [self.cropFilter removeAllTargets];
    [self.faceFilter removeAllTargets];
    [self.widgetImg1 removeAllTargets];
    [self.widgetImg2 removeAllTargets];
    [self.widgetImg3 removeAllTargets];
    [self.widgetImg4 removeAllTargets];
    [self.widgetImg5 removeAllTargets];
    [self.widgetImg6 removeAllTargets];
...
- (void)willOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer{
    ...
    if(!self.detectoredFace && (now - self.lastTrackerTime) < 0.3){
        return;
    }
    [self grepFacesForSampleBuffer:sampleBuffer];
 
    self.lastTrackerTime = now;
    int i = 0;
    ...
    for (NSDictionary *item in self.stickerConfig[@"items"]) {
        ...
        if([item[@"type"] integerValue] == 1 ){
            ...
            if(!self.mouthOpening){
                [self.widgetImg1 removeAllTargets];
                self.widgetImg1 = self.placeholderImg;
                [self.widgetImg1 addTarget:self.peopleFilter atTextureLocation:1];
                [self.widgetImg1 processImage];
                i++;
                continue;
            }
        }
        ...
        if(i == 0){
            [self.widgetImg1 removeAllTargets];
            self.widgetImg1 = itemImg;
            [self.widgetImg1 addTarget:self.peopleFilter atTextureLocation:1];
            [self.widgetImg1 processImage];
        }else if(i == 1){
            [self.widgetImg2 removeAllTargets];
            self.widgetImg2 = itemImg;
            [self.widgetImg2 addTarget:self.peopleFilter2 atTextureLocation:1];
            [self.widgetImg2 processImage];
        }else if(i == 2){
            [self.widgetImg3 removeAllTargets];
            self.widgetImg3 = itemImg;
            [self.widgetImg3 addTarget:self.peopleFilter3 atTextureLocation:1];
            [self.widgetImg3 processImage];
        }else if(i == 3){
            [self.widgetImg4 removeAllTargets];
            self.widgetImg4 = itemImg;
            [self.widgetImg4 addTarget:self.peopleFilter4 atTextureLocation:1];
            [self.widgetImg4 processImage];
        }else if(i == 4){
            [self.widgetImg5 removeAllTargets];
            self.widgetImg5 = itemImg;
            [self.widgetImg5 addTarget:self.peopleFilter5 atTextureLocation:1];
            [self.widgetImg5 processImage];
        }else if(i == 5){
            [self.widgetImg6 removeAllTargets];
            self.widgetImg6 = itemImg;
            [self.widgetImg6 addTarget:self.peopleFilter6 atTextureLocation:1];
            [self.widgetImg6 processImage];
        }
        i++;
    }
...
- (void)grepFacesForSampleBuffer:(CMSampleBufferRef)sampleBuffer{
    ...
    NSArray *faces = [self.facear landmark:gray_image scale:scale lowModel:self.lowModel isFrontCamera:self.isUsingFrontFacingCamera];
    gray_image.release();
...
- (void)GPUVCWillOutputFeatures:(NSArray *)faceArray
{
    ...
    for (NSDictionary *item in self.stickerConfig[@"items"]) {
        ...
        if(i == 0){
            ...
            [self.peopleFilter5 setStickerParams:faceParameArray[i]];
        }else if(i == 5){
            [self.peopleFilter6 setStickerParams:faceParameArray[i]];
        }
        i++;
    }
}
